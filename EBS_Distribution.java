...
public class EBS_Distribution {

  public static void main(String[] args){

		if(args.length != 3){
			System.out.println("Please call the program like this : java EBS_Distribution <k> <m> <number_Of_Nodes_Hacked>");
			System.exit(0);
		}
    ...
    int xLength=1000, yWidth=1000; //meters
    ...
    try
    {
      ...
      //Condition to have an EBS system
			...
			Nmax=(int)(binomial((long)(k+m), (long)(k)));
			N=(int)(binomial((long)(m), (long)(k)));
			
			//System.out.println("The maximum of nodes for this paramaters (k:"+k+",m:"+m+") is : "+Nmax);

      //calcul number keys
			Counter counterCoeffs = new Counter();
      counterCoeffs.incr();
			double resDouble=0;
			while(counterCoeffs.getValue()<Nmax && resDouble==0) {
				if(Math.pow(2,(double)(counterCoeffs.getValue()))>Nmax) {
					resDouble=Math.pow(2,(double)(counterCoeffs.getValue()-1));
				}
				counterCoeffs.incr();
			}
			counterCoeffs.decr();
      coeffSizeTabKeys=counterCoeffs.getValue();
      ...
      for(int i=0; i<number_Of_Nodes; i++) {
        sensors[i] = new Node(i, Math.abs(mt.nextInt(xLength+1)%1000), Math.abs(mt.nextInt(yWidth+1)%1000), number_Of_Nodes);
      ...
      for(int i=0; i<bunchKey.length; i++) {
        ...
        //System.out.println(bunchKey[i].toString());
			...
			getCombination(uniqueCombination, k-1, number_Of_Nodes, str); //k-1 pour avoir taille k pour une combinaisons
	    ...
	    System.out.println("I have a result of : " + ratio+" for the conectivity\n");
  ...
  public static void increment(List<Boolean> bytes) {
	...
	public static String combination(String possibleChars, int charsToTake) {
	...
	public static void getCombination(List<String> uniqueCombination, int size, int numberOfComb, String string) {
		String resStr;
		int counterC, counterS;
		for(int i=0; i<numberOfComb; i++) {
			resStr="";
			resStr=combination(string, size);
			counterS=0;
			for(String s : uniqueCombination) {
				counterC=0;
				for(int j=0; j<size; j++) {
					for(int l=0; l<size; l++) {
						if(resStr.charAt(j)==s.charAt(l)) {
							counterC++;
						}
					}
				}
				if(counterC==size) counterS++;
			}
			if(counterS!=0)
				i--;
			else uniqueCombination.add(resStr);
		}
	}
